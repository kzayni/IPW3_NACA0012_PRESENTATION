...
\begin{block}{\scriptsize Observations}
\begin{itemize}
    \item Impingement limits: $\beta=0.001$ crossings;
    ...
    with isolated deviations up to $\sim4-6\%$.
    \item No consistent grid-convergence trend is observed across submissions.
\end{itemize}
\end{block}
...
\begin{itemize}
    \item The 7-bin distribution closely reproduces the 15-bin results, while the
    single-bin overpredicts $\beta_{\max}$ and underpredicts impingement width
    and collected water mass.
\end{itemize}
\end{block}
